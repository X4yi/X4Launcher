...
CLion can open the project directly through `CMakePresets.json`.
...
Useful environment variables for CLion profiles:

```bash
# Native Qt
export X4_QT_ROOT=/path/to/qt

# Windows cross-compiling with MinGW-w64
export X4_MINGW_TRIPLET=x86_64-w64-mingw32
export X4_MINGW_ROOT=/usr/x86_64-w64-mingw32
export X4_MINGW_BIN_DIR=/usr/bin
export X4_QT_ROOT=/path/to/windows/qt/prefix
export X4_QT_HOST_PATH=/path/to/native/qt/prefix
```

Notes:

* `X4_QT_ROOT` must point to a prefix that contains `lib/cmake/Qt6`.
* Cross builds need both the Windows Qt target files and a native Qt host install for tools like `moc`.
* The MinGW toolchain file no longer depends on the compiler being discoverable only through `PATH`.

---

## Distribution

The project includes a `distribute.sh` script to automate the packaging process for Linux and Windows (via cross-compilation).

### Linux
To create a portable `.tar.gz` package for Linux:
```bash
./distribute.sh --linux
```

### Windows (Cross-compilation)
To build for Windows from Linux, you need `mingw-w64` installed. Then run:
```bash
...
* `--clean`: Remove previous build/dist artifacts before starting.

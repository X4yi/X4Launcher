...
namespace x4::auth {
    ...
    void AuthController::login() {
        ...
        QUrl url = msClient_->buildAuthorizeUrl(ClientId, redirectUri, currentSession_->state(),
                                                currentSession_->pkceChallenge(),
                                                currentSession_->pkceChallengeMethod());
    ...
    void AuthController::onCodeReceived(const QString &code, const QString &state) {
        ...
        if (!currentSession_ || state != currentSession_->state()) {
            qDebug() << "[Auth] State mismatch! Expected:" << (currentSession_
                                                                   ? currentSession_->state()
                                                                   : "(no session)");

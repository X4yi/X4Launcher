...
namespace x4::auth {
    ...
    void MinecraftAuthClient::loginWithXbox(const QString &xstsToken, const QString &userHash) {
        ...
        qDebug() << "[Auth] MC login identityToken prefix:" << QStringLiteral("XBL3.0 x=%1;%2...").arg(
            userHash, xstsToken.left(20));
    ...
    void MinecraftAuthClient::onLoginReply() {
        ...
        if (reply->error() != QNetworkReply::NoError) {
            emit errorOccurred(
                QStringLiteral("Minecraft Login Error (HTTP %1): %2").arg(httpStatus).arg(
    ...
    void MinecraftAuthClient::fetchProfile(const QString &mcAccessToken, int expiresIn) {
        ...
        connect(reply, &QNetworkReply::finished, this, [this, reply, mcAccessToken, expiresIn]() {
            ...
            if (error.error != QJsonParseError::NoError) {
                emit errorOccurred(
                    QStringLiteral("Failed to parse Minecraft profile response: %1").arg(error.errorString()));
                return;
            }

...
namespace x4 {
    ...
    Result<Instance> InstanceImporter::importZip(const QString &zipPath) {
        ...
        if (!sourceMcDir.isEmpty()) {
            ...
            X4_WARN("Import", QStringLiteral(
                        "No .minecraft directory found in the archive. Instance created without game files."));
        }
    ...
    bool InstanceImporter::extractZip(const QString &zipPath, const QString &tempDir, QString &outError) {
        ...
        proc.start(QStringLiteral("unzip"), {
                       QStringLiteral("-q"), QStringLiteral("-o"), zipPath, QStringLiteral("-d"), tempDir
                   });
        if (!proc.waitForStarted(5000)) {
            // Fallback: try 7z
            proc.start(QStringLiteral("7z"), {
                           QStringLiteral("x"), QStringLiteral("-y"), QStringLiteral("-o") + tempDir, zipPath
                       });
            if (!proc.waitForStarted(5000)) {

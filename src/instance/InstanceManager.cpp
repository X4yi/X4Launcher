...
QString InstanceManager::instanceDir(const QString& id) const {
    // Find the instance by ID to get its name
    if (const Instance* inst = findById(id)) {
        ...
        // Handle empty names
        if (safeName.isEmpty()) {
            safeName = QStringLiteral("instance_%1").arg(id.left(8));
        }
        return PathManager::instance().instancesDir() + QStringLiteral("/") + safeName;
    }
    // Fallback to UUID if instance not found
    return PathManager::instance().instancesDir() + QStringLiteral("/") + id;
}
...
Result<Instance> InstanceManager::createInstance(const QString& name, const QString& mcVersion,
    ...
    QString dir = instanceDir(inst.id);

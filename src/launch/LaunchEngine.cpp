...
bool isSafeChildPath(const QString& basePath, const QString& childPath) {
    ...
    if (!childCanon.isEmpty()) {
        ...
        return childCanon.startsWith(baseCanon + QDir::separator());
    }
    ...
    return childAbs.startsWith(baseAbs + QDir::separator());
}

bool safeRemoveRecursively(const QString& basePath, const QString& targetPath) {
    if (!isSafeChildPath(basePath, targetPath)) {
        X4_ERROR("Launch", QStringLiteral("Refusing to remove path outside of base: %1 (base=%2)").arg(targetPath, basePath));
        return false;
    }
    return QDir(targetPath).removeRecursively();
}
bool isWaylandSession(const QProcessEnvironment& env) {
    return env.value(QStringLiteral("XDG_SESSION_TYPE"), QString()).compare(QStringLiteral("wayland"), Qt::CaseInsensitive) == 0
...
QString detectKnownCrashReason(const QStringList& lines) {
    for (const QString& line : lines) {
        ...
        if (line.contains(QStringLiteral("Unable to launch"), Qt::CaseInsensitive)) {
            return QStringLiteral("LaunchWrapper no pudo iniciar Minecraft. Revisa stacktrace y dependencias/mods.");
        }
    }
    return {};
}
...
void LaunchEngine::fetchLoaderMeta(const QString& instanceId, const QByteArray& vanillaData) {
    ...
    QString uid;
    switch (inst->loader) {
        case ModLoaderType::Fabric: uid = "net.fabricmc.fabric-loader"; break;
        case ModLoaderType::Forge: uid = "net.minecraftforge"; break;
        case ModLoaderType::NeoForge: uid = "net.neoforged"; break;
        case ModLoaderType::Quilt: uid = "org.quiltmc.quilt-loader"; break;
        default: break;
    }
    ...
    auto handlePatchJson = [this, meta, instanceId, vanillaData, inst](const QByteArray& patchData) {
        ...
        if (patchResult.isErr()) {
            emit launchFailed(instanceId, QStringLiteral("Failed to parse mod loader JSON: %1").arg(patchResult.error()));
            return;
        }
    ...
    if (inst->loaderVersion.isEmpty()) {
        connect(meta, &MetaAPI::indexFinished, this, [=](const QString&, const QByteArray& idxData) {
            ...
            for (auto item : versions) {
                ...
                for (auto reqItem : reqs) {
                    ...
                    if (r["uid"].toString() == "net.minecraft" && r["equals"].toString() == inst->mcVersion) {
            ...
            if (bestVersion.isEmpty()) {
                emit launchFailed(instanceId, QStringLiteral("No compatible %1 version found for Minecraft %2").arg(uid, inst->mcVersion));
                meta->deleteLater();
                return;
            }
            ...
            connect(meta, &MetaAPI::versionFinished, this, [=](const QString&, const QString&, const QByteArray& pdata) { handlePatchJson(pdata); });
            connect(meta, &MetaAPI::versionFailed, this, [=](const QString&, const QString&, const QString& err) { emit launchFailed(instanceId, "Mod Loader DL Error: " + err); meta->deleteLater(); });
            meta->getPackageVersion(uid, bestVersion);
        });
        
        connect(meta, &MetaAPI::indexFailed, this, [=](const QString&, const QString& err) {
    ...
    if (dm->totalCount() == 0) {
        ...
        if (QFile::exists(assetIndexPath)) {
            ...
            if (f.open(QIODevice::ReadOnly)) {
                ...
                if (dm2->totalCount() > 0) {
                    emit launchProgress(instanceId, QStringLiteral("Downloading assets..."));
                    connect(dm2, &DownloadManager::allFinished, this,
                            [this, dm2, instanceId, versionInfo](bool success, const QStringList& errors) {
                                dm2->deleteLater();
                                if (!success) {
                                    X4_WARN("Launch", QStringLiteral("Some assets failed: %1").arg(errors.join(", ")));
                                }
            ...
            [this, dm, instanceId, versionInfo](bool success, const QStringList& errors) {
                ...
                if (QFile::exists(assetIndexPath)) {
                    ...
                    if (f.open(QIODevice::ReadOnly)) {
                        ...
                        if (dm2->totalCount() > 0) {
                            emit launchProgress(instanceId,
                                QStringLiteral("Downloading %1 assets...").arg(dm2->totalCount()));
                            connect(dm2, &DownloadManager::allFinished, this,
...
void LaunchEngine::doLaunch(const QString& instanceId, const VersionInfo& versionInfo) {
    ...
    if (!manifestEntry) {
        emit launchFailed(instanceId, QStringLiteral("GAME_LAUNCH_ERROR: Minecraft version %1 not found in manifest.").arg(inst->mcVersion));
        return;
    }
    ...
    if (!configJavaPath.isEmpty()) {
        ...
        if (!candidate.isValid()) {
            emit launchFailed(instanceId, QStringLiteral("GAME_LAUNCH_ERROR: Configured Java path is invalid: %1").arg(configJavaPath));
            return;
        }
        const int required = JavaManager::requiredJavaMajor(inst->mcVersion);
        if (candidate.major < required) {
            emit launchFailed(instanceId, QStringLiteral("GAME_LAUNCH_ERROR: Configured Java (%1) is too old for MC %2 (needs Java %3+).")
    ...
    if (!java.isValid()) {
        emit launchFailed(instanceId,
            QStringLiteral("GAME_LAUNCH_ERROR: No suitable Java found for Minecraft %1 (needs Java %2+)")
    ...
    if (inst->loader == ModLoaderType::Forge || inst->loader == ModLoaderType::NeoForge) {
        if (origGameDir.contains(QStringLiteral("+")) || origGameDir.contains(QStringLiteral("!"))) {
            ...
            if (QFile::link(origGameDir, linkPath)) {
                created = true;
                X4_INFO("Launch", QStringLiteral("QFile::link created symlink %1 -> %2").arg(linkPath, origGameDir));
            } else {
            ...
            if (created) {
                ...
                X4_INFO("Launch", QStringLiteral("Created safe symlink for instance %1 -> %2").arg(linkPath, origGameDir));
            } else {
                X4_WARN("Launch", QStringLiteral("Forge path contains invalid characters and safe symlink creation failed: %1").arg(origGameDir));
                emit launchFailed(instanceId,
                    QStringLiteral("Forge cannot launch from a folder containing characters like '+' or '!'.\n\n"
    ...
    if (QStandardPaths::findExecutable(QStringLiteral("xrandr")).isEmpty()) {
        ...
        if (!isSafeChildPath(nativesBase, nativesDir)) {
            X4_WARN("Launch", QStringLiteral("Refusing to create dummy xrandr: unsafe nativesDir %1").arg(nativesDir));
        } else {
            ...
            if (!fx.exists() && fx.open(QIODevice::WriteOnly | QIODevice::Text)) {
                ...
                fx.close();
                fx.setPermissions(fx.permissions() | QFileDevice::ExeOwner | QFileDevice::ExeUser | QFileDevice::ExeGroup | QFileDevice::ExeOther);
    ...
    if (wayland) {
        ...
        if (binding.display.isEmpty()) {
            const bool allowEmbeddedXwayland = env.value(QStringLiteral("X4_ALLOW_EMBEDDED_XWAYLAND")) == QStringLiteral("1");
            if (allowEmbeddedXwayland) {
                QString startedDisplay;
                QString startReason;
                X4_WARN("Launch", QStringLiteral("No usable compositor XWayland found. Trying embedded Xwayland (debug mode)..."));
        ...
        X4_INFO("Launch", QStringLiteral("Wayland detected -> forcing XWayland on DISPLAY %1").arg(binding.display));
    } else {
...
bool LaunchEngine::extractNatives(const QString& nativesDir, const QVector<LibraryInfo>& libs, QString* error) {
    for (const auto& lib : libs) {
...
#else
        proc.start(QStringLiteral("unzip"), {QStringLiteral("-o"), QStringLiteral("-q"), jarPath, QStringLiteral("-d"), tmpDir.path()});
#endif
        ...
        while (it.hasNext()) {
            ...
            if (rel.startsWith(QStringLiteral("META-INF"), Qt::CaseSensitive) || rel.startsWith(QStringLiteral("__MACOSX"))) {
                continue;
            }

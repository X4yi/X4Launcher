...
namespace x4 {
    ...
    void ModrinthAPI::searchProjects(const QString &query, const QString &facets, int limit, int offset,
                                     const QString &index) {
    ...
    ModrinthVersion ModrinthAPI::parseVersion(const QJsonObject &obj) {
        ...
        v.projectId = obj["project_id"].toString();
    ...
    void ModrinthAPI::batchGetProjects(const QStringList &projectIds) {
        QUrl url(API_URL + "/projects");
        QUrlQuery q;
        QJsonArray ids;
        for (const auto &pid: projectIds) ids.append(pid);
        q.addQueryItem("ids", QString::fromUtf8(QJsonDocument(ids).toJson(QJsonDocument::Compact)));
        url.setQuery(q);
        ...
        connect(reply, &QNetworkReply::finished, this, [this, reply]() {
            reply->deleteLater();
            if (reply->error() != QNetworkReply::NoError) {
                emit batchProjectsFailed(reply->errorString());
                return;
            }
            ...
            auto doc = QJsonDocument::fromJson(reply->readAll(), &err);
            if (err.error != QJsonParseError::NoError || !doc.isArray()) {
                emit batchProjectsFailed("Invalid JSON response from batch projects");
                return;
            }

            auto arr = doc.array();
            QMap<QString, ModrinthProject> resultMap;
            for (const auto &val: arr) {
                auto p = parseProject(val.toObject());
                resultMap.insert(p.id, p);
            }

            emit batchProjectsFinished(resultMap);
        });
    }
...
#include <functional>
    ...
    private:
        ...
        void processNext() {
            if (!pendingVersions_.isEmpty()) {
                ...
                connect(reply, &QNetworkReply::finished, this, [this, reply, vid]() {
                    reply->deleteLater();
                    if (reply->error() != QNetworkReply::NoError) {
            ...
            if (!pendingProjects_.isEmpty()) {
                ...
                if (!mcVersion_.isEmpty()) {
                    QJsonArray vers;
                    vers.append(mcVersion_);
                    q.addQueryItem("game_versions",
                                   QString::fromUtf8(QJsonDocument(vers).toJson(QJsonDocument::Compact)));
                }
                if (loader_ != ModLoaderType::Vanilla) {
                    QJsonArray loads;
                    loads.append(modLoaderName(loader_).toLower());
                    q.addQueryItem("loaders", QString::fromUtf8(QJsonDocument(loads).toJson(QJsonDocument::Compact)));
                }
                url.setQuery(q);
    ...
    void ModrinthAPI::resolveDependencies(const QStringList &versionIds, const QString &mcVersion,

...
namespace x4 {
    InstanceEditorDialog::InstanceEditorDialog(InstanceManager *mgr, JavaManager *javaMgr,
                                               ...
                                               QWidget *parent)
        : QDialog(parent), mgr_(mgr), javaMgr_(javaMgr), manifest_(manifest), launchEngine_(launchEngine),
                ...
                [this](const QString &key) {
                    if (key == QStringLiteral("java/path")) {
                        javaPathEdit_->setPlaceholderText(QStringLiteral("(use global default: %1)")
                            .arg(SettingsManager::instance().defaultJavaPath().isEmpty()
                                     ? QStringLiteral("auto-detect")
                                     : SettingsManager::instance().defaultJavaPath()));
                    }
    ...
    void InstanceEditorDialog::setupUI() {
        ...
        launchBtn->setStyleSheet(QStringLiteral(
            "QPushButton { color: #9ece6a; font-weight: bold; } QPushButton:hover { background-color: #1a2e1a; }"));
        ...
        killBtn->setStyleSheet(QStringLiteral(
            "QPushButton { color: #f7768e; font-weight: bold; } QPushButton:hover { background-color: #3b2030; }"));
    ...
    QWidget *InstanceEditorDialog::createOverviewTab() {
        ...
        iconCombo_->addItem(QIcon(QStringLiteral(":/icons/vanilla_icon.png")), QStringLiteral("Vanilla"),
                            QStringLiteral("vanilla_icon"));
        iconCombo_->addItem(QIcon(QStringLiteral(":/icons/steve_icon.png")), QStringLiteral("Steve"),
                            QStringLiteral("steve_icon"));
        iconCombo_->addItem(QIcon(QStringLiteral(":/icons/forge_icon.png")), QStringLiteral("Forge"),
                            QStringLiteral("forge_icon"));
        iconCombo_->addItem(QIcon(QStringLiteral(":/icons/fabric_icon.png")), QStringLiteral("Fabric"),
                            QStringLiteral("fabric_icon"));
        iconCombo_->addItem(QIcon(QStringLiteral(":/icons/creeper_icon.png")), QStringLiteral("Creeper"),
                            QStringLiteral("creeper_icon"));
        iconCombo_->addItem(QIcon(QStringLiteral(":/icons/chicken_icon.png")), QStringLiteral("Chicken"),
                            QStringLiteral("chicken_icon"));
        iconCombo_->addItem(QIcon(QStringLiteral(":/icons/goat_icon.png")), QStringLiteral("Goat"),
                            QStringLiteral("goat_icon"));
        iconCombo_->addItem(QIcon(QStringLiteral(":/icons/cat_icon.png")), QStringLiteral("Cat"),
                            QStringLiteral("cat_icon"));
    ...
    QWidget *InstanceEditorDialog::createResourcePacksTab() {
        ...
        connect(resourcePackList_, &QListWidget::customContextMenuRequested, this,
                &InstanceEditorDialog::showItemContextMenu);
    ...
    QWidget *InstanceEditorDialog::createShaderPacksTab() {
        ...
        connect(shaderPackList_, &QListWidget::customContextMenuRequested, this,
                &InstanceEditorDialog::showItemContextMenu);
    ...
    void InstanceEditorDialog::refreshModList() {
        modList_->clear();
        QString dir = mgr_->minecraftDir(instanceId_) + QStringLiteral("/mods");
        populateRichFileList(modList_, dir, {
                                 QStringLiteral("*.jar"), QStringLiteral("*.zip"), QStringLiteral("*.disabled")
                             });
    }
    ...
    void InstanceEditorDialog::populateRichFileList(QListWidget *list, const QString &dir, const QStringList &filters) {
        ...
        connect(watcher, &QFutureWatcher<QMap<QString, QString> >::finished, this, [this, watcher, list, fileItems]() {
                ...
                if (metadataCache_.contains(hash)) {
                    ...
                    if (item) {
                        ...
                        if (w) {
                            ...
                            descLbl->setText(
                                QString("Project ID: <a href='https://modrinth.com/project/%1'>%1</a> | Version: %2")
                                .arg(ver.projectId)
            ...
            connect(modrinthApi_, &ModrinthAPI::batchHashesFinished, this,
                    [this, list, fileItems, hashToFile, fileToHash, hasherList](
                const QMap<QString, x4::ModrinthVersion> &res) {
                        ...
                        if (projectIds.isEmpty()) {
                            ...
                            for (auto it = res.begin(); it != res.end(); ++it) {
                                ...
                                if (item) {
                                    auto *w = list->itemWidget(item);
                                    if (w) {
                                        auto *textLayout = qobject_cast<QVBoxLayout
                                            *>(w->layout()->itemAt(1)->layout());
                                        auto *titleLbl = qobject_cast<QLabel *>(textLayout->itemAt(0)->widget());
                                        auto *descLbl = qobject_cast<QLabel *>(textLayout->itemAt(1)->widget());
                                        titleLbl->setText(QString("<b>%1</b>").arg(meta.name));
                                        descLbl->setText(
                                            QString("Project ID: %1 | Version: %2").arg(meta.projectId).arg(
                                                meta.versionNumber));
                                    }
                                }
                            }
                        ...
                        connect(modrinthApi_, &ModrinthAPI::batchProjectsFinished, this,
                                [this, res, list, fileItems, hashToFile, fileToHash](
                            const QMap<QString, x4::ModrinthProject> &projects) {
                                    for (auto it = res.begin(); it != res.end(); ++it) {
                                        QString hash = it.key();
                                        auto ver = it.value();
                                        QString file = hashToFile.value(hash);
                                        ...
                                        if (item) {
                                            ...
                                            if (w) {
                                                auto *layout = w->layout();
                                                auto *textLayout = qobject_cast<QVBoxLayout *>(
                                                    layout->itemAt(1)->layout());
                                                auto *titleLbl = qobject_cast<QLabel
                                                    *>(textLayout->itemAt(0)->widget());
                                                auto *descLbl = qobject_cast<QLabel *>(textLayout->itemAt(1)->widget());

                                                titleLbl->setText(QString("<b>%1</b>").arg(meta.name));
                                                descLbl->setText(
                                                    QString(
                                                        "Project ID: <a href='https://modrinth.com/project/%1'>%1</a> | Version: %2")
                                                    .arg(meta.projectId)
                                    ...
                                    for (auto *item: fileItems.values()) {
                                        ...
                                        if (!res.contains(fileToHash.value(file))) {
                                            ...
                                            if (w) {
                                                ...
                                                auto *descLbl = qobject_cast<QLabel *>(textLayout->itemAt(1)->widget());
                                                descLbl->setText(
                                                    QStringLiteral("<i>Not found on Modrinth (Local file)</i>"));
                                                CachedMetadata meta;
    ...
    void InstanceEditorDialog::deleteSelectedItems() {
        ...
        QTimer::singleShot(0, this, [this, list, items]() {
            if (QMessageBox::question(this, X4TR("editor.msg.del.title"), X4TR("editor.msg.del.desc")) !=
                QMessageBox::Yes) {
                return;
            }
    ...
    void InstanceEditorDialog::updateSelectedItems() {
        QTimer::singleShot(0, this, [this]() {
            QMessageBox::information(this, X4TR("editor.msg.update.title"), X4TR("editor.msg.update.desc"));
        });
    }

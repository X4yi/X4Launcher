...
ModrinthBrowserDialog::ModrinthBrowserDialog(const QString &instanceId,
                                             InstanceManager *instanceMgr,
                                             const QString &initialFilter,
                                             QWidget *parent)
    : QDialog(parent), instanceId_(instanceId), instanceMgr_(instanceMgr) {
  ...
  searchTimer_->setInterval(400);

  connect(api_, &ModrinthAPI::searchFinished, this,
          &ModrinthBrowserDialog::onSearchFinished);
  connect(api_, &ModrinthAPI::searchFailed, this,
          &ModrinthBrowserDialog::onSearchFailed);
  connect(api_, &ModrinthAPI::versionsFinished, this,
          &ModrinthBrowserDialog::onVersionsFinished);
  connect(api_, &ModrinthAPI::versionsFailed, this,
          &ModrinthBrowserDialog::onVersionsFailed);
  connect(api_, &ModrinthAPI::dependenciesResolved, this,
          &ModrinthBrowserDialog::onDependenciesResolved);
  connect(api_, &ModrinthAPI::dependenciesResolveFailed, this,
          &ModrinthBrowserDialog::onDependenciesResolveFailed);
  connect(searchTimer_, &QTimer::timeout, this,
          &ModrinthBrowserDialog::performSearch);
...
void ModrinthBrowserDialog::setupUI() {
  ...
  versionsTable_->setHorizontalHeaderLabels(
      {"Version", "Game Versions", "Loaders"});
  versionsTable_->horizontalHeader()->setSectionResizeMode(
  ...
  connect(resultsList_, &QListWidget::itemDoubleClicked, this,
          [this](QListWidgetItem *item) {
            ...
            if (!currentSelectedProjectId_.isEmpty() &&
                !currentVersions_.isEmpty()) {
  ...
  connect(downloadQueueBtn_, &QPushButton::clicked, this,
          &ModrinthBrowserDialog::flushQueue);
}
...
void ModrinthBrowserDialog::performSearch() {
  ...
  QString facets = ModrinthAPI::buildFacets(
      currentInstance_.mcVersion, currentInstance_.loader, projectType);

  api_->searchProjects(query, facets, 30, 0, index);
}

void ModrinthBrowserDialog::onSearchFinished(
    const x4::ModrinthSearchResult &result) {
  resultsList_->clear();
  ...
  for (int i = 0; i < currentResults_.size(); ++i) {
    ...
    QString text =
        QString("%1\n%2 downloads").arg(title, QString::number(p.downloads));
...
void ModrinthBrowserDialog::onProjectClicked(QListWidgetItem *item) {
  ...
  if (idx < 0 || idx >= currentResults_.size())
    return;

  const auto &p = currentResults_[idx];

  if (p.id == currentSelectedProjectId_) {
    // Project already selected, just select the latest version in the table and
    // return
    if (versionsTable_->rowCount() > 0) {
  ...
  QString html =
      QString("<h2>%1</h2><p><b>By %2</b></p><hr><p>%3</p>")
  ...
  if (typeCombo_->currentData().toString() == "mod" &&
      currentInstance_.loader != ModLoaderType::Vanilla) {
...
void ModrinthBrowserDialog::onVersionsFinished(
    const QString &projectId, const QVector<x4::ModrinthVersion> &versions) {
  if (projectId != currentSelectedProjectId_)
    return;
...
void ModrinthBrowserDialog::onVersionsFailed(const QString &projectId,
                                             const QString &err) {
  if (projectId != currentSelectedProjectId_)
    return;
  versionsTable_->setRowCount(1);
  versionsTable_->setItem(
      0, 0, new QTableWidgetItem("Failed to load versions: " + err));
}

void ModrinthBrowserDialog::addToQueue(const x4::ModrinthVersion &version,
                                       const x4::ModrinthFile &file) {
  installVersionBtn_->setEnabled(false);
  installVersionBtn_->setText("Resolving...");
  ...
  api_->resolveDependencies({version.id}, currentInstance_.mcVersion,
                            loaderToUse);
}

void ModrinthBrowserDialog::onDependenciesResolved(
    const QVector<x4::ModrinthVersion> &deps) {
  installVersionBtn_->setText("Add to Queue");
  if (versionsTable_->selectedItems().count() > 0) {
    installVersionBtn_->setEnabled(true);
  }
  ...
  QString targetBaseDir =
      instanceMgr_->minecraftDir(instanceId_) + "/" + subDir;
  ...
  for (const auto &v : deps) {
    ...
    QString targetFile = targetBaseDir + "/" + file.filename;

    bool dup = false;
    for (const auto &existing : downloadQueue_) {
      if (existing.sha1 == file.sha1 || existing.filename == file.filename ||
...
void ModrinthBrowserDialog::onDependenciesResolveFailed(const QString &err) {
  installVersionBtn_->setText("Add to Queue");
  if (versionsTable_->selectedItems().count() > 0) {
    installVersionBtn_->setEnabled(true);
  }
  QMessageBox::warning(this, "Dependency Error",
                       "Failed to resolve dependencies: " + err);
}
...
void ModrinthBrowserDialog::flushQueue() {
  ...
  connect(dm, &DownloadManager::taskProgress, this,
          [](const QString &, qint64 recv, qint64 total) {
            if (total > 0) {
              QString speedStr = QString("%1 / %2 MB")
                                     .arg(recv / 1024.0 / 1024.0, 0, 'f', 1)
                                     .arg(total / 1024.0 / 1024.0, 0, 'f', 1);
              GlobalEvents::instance().downloadProgress(
                  static_cast<int>((recv * 100) / total), speedStr);
            }
          });

  connect(dm, &DownloadManager::allFinished, this,
          [this, dm, targets](bool success, const QStringList &errs) {
            dm->deleteLater();

            if (success) {
              downloadQueue_.clear();
              updateQueueUI();
              GlobalEvents::instance().downloadFinished(
                  true, "Installed items successfully!");
            } else {
              for (const auto &path : targets)
                QFile::remove(path);
              GlobalEvents::instance().downloadFinished(
                  false, "Download failed: " + errs.join(", "));
